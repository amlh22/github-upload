...
void drawFuncs::introMessage() {
	...
	cout << "  j to enable/disable diagonal sticking" << endl;
	cout << "  t to enable/disable bouncing of particles" << endl;
}
...
void drawFuncs::handleKeypress(unsigned char key, int x, int y) {
	...
	case 'j':
	case 'J':  // include uppercase letter in case Caps Lock is left on by accident.
		sys->attachDiag = (sys->attachDiag == 0 ? 1 : 0);  // flip the attachDiag variable.
														   // e.g. 0 -> 1 or 1 -> 0
		cout << "diagonal sticking is " << (sys->attachDiag == 0 ? "OFF" : "ON") << endl;
		break;

	case '0':
		cout << "Number of particles = " << sys->getNumParticles() << "\tRadius = " << sys->getClusterRadius() << endl;
		break;

	case 't':
	case 'T':  // include uppercase letter in case Caps Lock is left on by accident.
		// flip the bounce enabled flag (OFF->ON or ON->OFF)
		sys->bounceEnabled = !sys->bounceEnabled;
		cout << "bouncing of particles is " << (sys->bounceEnabled ? "ON" : "OFF") << endl;
		break;
	}
