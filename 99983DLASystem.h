...
#define OUTPUT_FILE_NAME "projectoutput4000nondiag2bouncefinal.txt"
...
private:
	...
	// output file
	// this is used in "Exercise 2 - measuring the fractal dimension"
	ofstream logfile;
	...
	// probability with which a particle will stick to the cluster whenever it touches
	// an existing particle.
	// this is used in "Exercise 3".
	// if "bounceEnabled" is false, this value will be ignored and particles will always stick.
...
public:
	...
	// set to 1 if particles can attach to the growing cluster if they touch diagonally, 0 otherwise.
	// defaults to 0 (particles will not attach if they touch diagonally).
	// this is used in "Exercise 1 - conditions for attachment"
	int attachDiag;

	// should particles have a chance of bouncing after touching the cluster?
	// if this is false, particles that touch the cluster will always stick.
	// if this is true, "stickProbability" will control the stickiness of particles.
	//
	// for "Exercise 3", this should be set to true.
	bool bounceEnabled;
	...
	// check if given position (x, y) lies within the boundaries of our grid
	...
	// getter and setter for stickProbability
	double getStickProbability();
	void setStickProbability(double stickProbability);
	...
	// if bouncing is enabled and sticking probability is less than 1.0, a particle
	// can bounce off when it touches a cluster. decide whether this should happen.
	bool shouldBounceOff();

	// print the number of particles (Nc) and size (R) of the cluster to a file.
	// this is used in "Exercise 2 - measuring the fractal dimension"
	void writeToFile(int Nc, double R);
